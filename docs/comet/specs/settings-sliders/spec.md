...
| `particle_count` | `input[type="range"]` | 80 | 140 | 1 | integer |
| `trail_span` | `input[type="range"]` | 0.12 | 0.68 | 0.01 | two decimal places |
| `duration_ms` | `input[type="range"]` | 500 | 1500 | 1 | integer with `ms` unit |
| `pulse_duration_ms` | `input[type="range"]` | 500 | 2000 | 1 | integer with `ms` unit |
| `rotation_duration_ms` | `input[type="range"]` | 500 | 3000 | 1 | integer with `ms` unit |
| `stroke_width` | `input[type="range"]` | 1.0 | 5.0 | 0.1 | one decimal place |

`curve_id` remains a native `select`. `formula` remains a read-only `output`.
...
- No change to curve selection, color editing, localization architecture, integration actions, or renderer algorithms.
