...
- **THEN** the family thumbnail grid contains exactly the 10 families above in order, with their outlines, names and either a direct-use label for a single preset or a preset count for multiple presets directly visible
...
外观页 SHALL 在“曲线方案 / Curve scheme”区的当前曲线与更换入口之后显示当前预设的“曲线参数 / Curve parameters”原生滑块、独立参数重置和只读公式。几何控件与公式不得继续位于渲染器区。按下表组内顺序呈现；不适用的控件不得出现。原 20 个预设的 ID、名称与族内相对顺序保留。
