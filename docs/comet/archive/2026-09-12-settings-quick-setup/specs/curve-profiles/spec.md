...
The application SHALL retain the following 20 named presets and stable IDs. Users browse them through the 10 curve families below; each family preserves its members' relative order from this catalog:
...
## Requirement: Curve families

曲线选择 SHALL 按以下 10 个曲线族组织。一个预设恰好属于一个族；族图形网格与族名提供中英文文本。族归属属于目录元数据，从 `curve_id` 推导，不新增持久化字段或改变旧 ID。

| Family | 中文名称 | Ordered preset IDs |
| --- | --- | --- |
| Thinking | 思考轨迹 | original-thinking, thinking-five, thinking-nine |
| Rose Orbit | 玫瑰环轨 | rose-orbit |
| Rose | 玫瑰线 | rose-curve, rose-two, rose-three, rose-four |
| Lissajous | 利萨如曲线 | lissajous-drift |
| Lemniscate | 双纽线 | lemniscate-bloom |
| Hypotrochoid | 内旋轮线 | hypotrochoid-loop, three-petal-spiral, four-petal-spiral, five-petal-spiral, six-petal-spiral |
| Butterfly | 蝴蝶曲线 | butterfly-phase |
| Cardioid | 心形线 | cardioid-glow, cardioid-heart |
| Heart Wave | 心形波纹 | heart-wave |
| Spiral Search | 螺旋搜索 | spiral-search |

原预设继续代表带名称的几何参数与动画参数组合。Thinking、Rose 和 Petal Spiral 的参数变体共享现有计算能力；Hypotrochoid/Petal Spiral 以及两种 Cardioid 在同族中保留各自的呼吸、旋转、朝向和行进规则。族归并不改变任何已保存预设的结果，不引入新的可调常量。

### Scenario: Select a built-in profile

- **WHEN** the curve picker is opened from Halo
- **THEN** the family thumbnail grid contains exactly the 10 families above in order, with their outlines, names and preset counts directly visible; selecting either a single or multiple-preset candidate requires a separate explicit Apply action before persistence
- **AND** every one of the 20 retained preset IDs is reachable exactly once within its family, in the specified relative order
- **AND** the current entry identifies both the selected family and its named preset
- **AND** `fourier-flow` is absent
- **AND** the read-only formula output in its separate disclosure describes the applied profile
...
族归并和参数布局调整保留全部旧预设 ID、默认几何、默认动画值、时间单位与现有重置语义。
...
光环页 SHALL 在“高级调整 / Advanced adjustments”默认折叠区提供当前预设的“曲线参数 / Curve parameters”原生滑块与独立参数重置，公式在单独的 details 中以只读 output 呈现。当前曲线与更换入口保持明显可见，几何控件不混入六动画分组。按下表组内顺序呈现；不适用的控件不得出现。原 20 个预设的 ID、名称与族内相对顺序保留。
...
明确应用另一预设 SHALL 同时加载它的默认几何和现有六项动画默认值，其他设置不变；再次激活当前预设保持值且不多余保存。独立“重置曲线参数 / Reset curve parameters”只重置当前几何；现有“重置曲线动画”继续只重置六项动画。新控件输入、重置与曲线切换在模型、公式、预览和保存负载中保持一致。
...
初次载入、外部设置事件与较早保存结果 SHALL 保留正在进行的本地参数编辑。失败保存保留可重试值并显示既有错误状态，不宣称已保存；图库重试不可重新加载默认值。单纯浏览或选择候选不得触发保存，只有明确应用才加载。一次恢复的字段范围和外部变化失效条件遵守 Curve Preset Picker，不建立逐预设历史；Glow 与透明度、颜色、音频一样不属于曲线应用或恢复范围。
