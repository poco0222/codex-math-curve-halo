...
默认缩略图 SHALL 静止。悬停或键盘聚焦某一项时，最多该项播放局部动画，使用现有 renderer。新候选使用该曲线现有默认动画与几何参数，当前项使用用户当前动画与几何参数；外观页当前曲线的静态入口也反映当前有效几何值。预览使用独立的本地参数副本，能够在桌面悬浮层关闭时查看；预览不得写入共享设置、发送保存命令或改变桌面悬浮层。最后一次明确的悬停或键盘聚焦决定预览项；离开预览目标后恢复静态轮廓。
...
只有点击或按 Enter、Space 激活预设才 SHALL 应用选择。新曲线调用现有默认值解析能力，设置 `curve_id` 与粒子数量、尾迹跨度、循环时长、脉冲时长、旋转时长、描边宽度，并加载目标预设的全部可调几何默认值；不得硬编码旧规格中的时长。保存继续走现有共享设置与串行队列，保留未挂载字段与其他设置。
...
实现 SHALL 沿用现有暗色视觉、系统字体、中文与英文词典，以及共享设置、Tauri Bridge、命令和事件。仅为 Curve Profiles 中规定的专属几何参数扩展 AppSettings；预览本身不增加持久化字段。继续保留曲线数学公式、默认外观、状态和配色数量、插件集成与图库结构，不添加搜索收藏分类或新第三方依赖。
