...
Appearance 页使用原生 range slider 调整透明度和连续动画参数，同时保留离散选择和只读公式的语义。光环位置通过桌面拖拽调整，不提供 X/Y 偏移控件。
...
`curve_id` remains a native `select`. `formula` remains a read-only `output`. Selecting a curve loads its six reference animation values; sliders save actual overrides without sentinel values. A localized current-curve reset restores those six values only. See Curve Profiles for the preset catalog and compatibility rules. `offset_x` and `offset_y` have no sliders, labels or value outputs; position behavior belongs to Overlay Positioning.
