...
在 **光环 → 音频可视化** 中开启 **系统音频**，即可使用默认声音输出
驱动当前曲线。低频伸缩曲线，中频改变粒子速度，高频加入有界细节。
**强度** 范围为 0～100%，默认 50%；0% 时仍显示真实音量反馈。

采集默认关闭。macOS 14.2+ 使用 Core Audio taps；Windows 10 1703+ / Windows 11
使用 WASAPI 回环采集。macOS 首次提示时允许系统音频采集；拒绝后可按设置中的
权限说明恢复并重试。其他输出设备及受保护音频可能无法采集。

音频仅在本机内存内处理，不录制、不上传。隐藏悬浮层或开启系统“减少动态效果”
会暂停采集；重新显示时按已保存的开关恢复。静音或输入中断后，曲线平滑回到原动画。

Windows 采集已实现，但因暂无验收环境，本轮 Windows 运行验收延期。
macOS 检查不代表 Windows 运行行为已验证。
...
设置分为 **光环** 与 **连接与运行**。光环页可以选择曲线、查看本地示例状态并调整
该状态颜色；透明度和光晕直接可用，几何、动画及公式在高级折叠区。图库中的选择
仅用于浏览，点击应用后才加载曲线及动画默认值。修改会自动保存，失败时可重试。

加载曲线后可一次恢复切换前的曲线与六项动画参数，保留最新的配色、透明度、光晕、
音频、位置、语言及运行设置。新的加载会替换恢复点，窗口隐藏或收到实际改变相关
参数的外部更新后恢复点失效；不保存历史方案。

本地状态预览不会改变桌面的真实任务状态。**连接与运行 → 高级诊断 → 桌面测试**
保留真实桌面模拟，新的实际事件可覆盖测试结果。桌面测试成功不代表插件已接通；
没有新事件也不代表连接失败。连接页提供接入说明、现有安装/卸载操作及最后已知事件。
